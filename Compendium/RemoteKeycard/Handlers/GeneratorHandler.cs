...
public static class GeneratorHandler
{
	...
	[Patch(typeof(Scp079Generator), "ServerInteract", PatchType.Prefix, new Type[] { })]
	private static bool GeneratorInteractionReplacement(Scp079Generator __instance, ReferenceHub ply, byte colliderId)
	{
		...
		try
		{
			if (__instance._cooldownStopwatch.IsRunning && __instance._cooldownStopwatch.Elapsed.TotalSeconds < (double)__instance._targetCooldown)
			{
				return false;
			}
			...
			case 0:
			{
				...
				{
					__instance.ServerSetFlag(Scp079Generator.GeneratorFlags.Unlocked, state: true);
			...
			case 2:
				...
				{
					__instance.ServerSetFlag(Scp079Generator.GeneratorFlags.Activating, state: false);
					__instance._targetCooldown = __instance._unlockCooldownTime;
					__instance._lastActivator = default(Footprint);
				}
				break;
